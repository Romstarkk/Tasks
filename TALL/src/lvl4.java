public class lvl4 {
    ...
    public static String formate(int n, int k, String str) {
        ...
        for (int i = 0; i < n && e != -1; i++) {
            ...
            if (line.length() - p + word.length() <= k) {
                line += word + " ";
                p++;
            } else {
                f += line.substring(0, line.length() - 1) + "\n";
                line = word + " ";
                p = 1;
            }
            s = e + 1;
    ...
    public static String BMI(String mass, String height) {
        ...
        if (mass.charAt(mass.indexOf(' ') + 1) == 'p') {
            m *= 0.453592f;
        }
        if (height.charAt(height.indexOf(' ') + 1) == 'i') {
            h *= 0.0254f;
        }
        m = m / (h * h);
    ...
    public static boolean doesRhyme(String str1, String str2) {
        ...
        for (int i = 0; i < c.length; i++) {
            if ((str1.contains(c[i] + "") ^ str2.contains(c[i] + ""))) {
                return false;
            }
        }
        return true;
    ...
    public static boolean trouble(long l1, long l2) {
        long t1, t2, t3 = l2;
        while (l1 > 0) {
            while (l1 % 10 != l1 % 1000 / 100 || l1 % 10 != l1 % 100 / 10) {
                l1 /= 10;
            }
            t1 = l1 % 10;
            l2 = t3;
            while (l2 > 0) {
                while (l2 % 10 != l2 % 100 / 10) {
                    l2 /= 10;
                }
                t2 = l2 % 10;
                if (t1 == t2) {
