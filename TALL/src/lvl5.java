...
public class lvl5 {
    ...
    public static boolean canMove(String figure, String pos, String des) {
        ...
        switch (figure) {
            ...
            case "Rook":
                ...
                break;
            case "Horse":
                r = ((Math.abs(dx) == 2 && Math.abs(dy) == 1) || (Math.abs(dy) == 2 && Math.abs(dx) == 1));
                break;
            case "Bishop":
                r = (Math.abs(dx) == Math.abs(dy));
                break;
            case "Queen":
                r = ((Math.abs(dx) == Math.abs(dy)) || (dx == 0 || dy == 0));
                break;
            case "King":
                r = (((Math.abs(dx) == Math.abs(dy)) || (dx == 0 || dy == 0)) && dx < 2 && dy < 2);
                break;
        }
        return r;
    }
    ...
    public static int sumDigProd(int[] ar) {
        int a = 0;
            ...
            if (str.charAt(i) == ' ' || str.length() - 1 == i) {
                ...
                switch (str.substring(s, e)) {
                    ...
                    case "and":
                        break;
                    default:
                        str = str.replace(str.substring(s, e), str.substring(s, s + 1).toUpperCase() + str.substring(s + 1, e));
                        break;
                }
                s = e + 1;
            }
